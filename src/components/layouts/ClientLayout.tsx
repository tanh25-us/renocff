...
import { ArrowLeft, CheckCircle, Minus, Plus, ShoppingBag, Trash2, X } from 'lucide-react';
...
function CheckoutView({ totalPrice, onBack, onSuccess, phone }: {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-y-auto">
      ...
      <div className="flex-1 space-y-5 overflow-y-auto p-5">
        ...
        {showQR && (
          <div className="rounded-2xl border border-[#d3c3bd] bg-white p-5 text-center">
            <p className="mb-3 text-sm font-bold">{method === 'Wallet' ? 'Quét mã MoMo' : 'Quét mã ngân hàng'}</p>
      ...
      <div className="border-t border-[#d3c3bd] p-5">
        <button onClick={onSuccess} className="w-full rounded-xl bg-[#25160e] py-3.5 text-sm font-bold text-white hover:bg-[#3c2a21]">
          Xác nhận đặt hàng
        </button>
      </div>
    </div>
...
export default function ClientLayout() {
  const { currentCustomer, logoutCustomer: _logout, addOrder } = useStore();
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative flex w-full max-w-sm flex-col bg-white shadow-2xl">
            ...
            {step === 'checkout' && currentCustomer && (
              <CheckoutView totalPrice={totalPrice} phone={currentCustomer.phone}
