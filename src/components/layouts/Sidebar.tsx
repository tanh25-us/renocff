import { BarChart3, Building2, Coffee, LayoutDashboard, ShoppingBag, Users, X } from 'lucide-react';
...
const APP_ROUTES = [
  { id: 'storefront', label: 'Cửa hàng', icon: Coffee },
...
export default function Sidebar({ currentTab, setTab, isOpen, onClose }: SidebarProps) {
  ...
  const handleClick = (tab: AppRouteId) => {
    setTab(tab);
    onClose();
  };
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex w-64 shrink-0 flex-col border-r border-white/10 bg-[#25160e] text-white transition-transform duration-300 md:static md:translate-x-0 ${
      ...
      >
        <div className="flex h-20 items-center justify-between border-b border-white/10 px-5">
          <div>
            ...
            <p className="label-caps mt-1 text-[#dec1b3]">Artisanal Management</p>
          </div>
            ...
            return (
              <button
                ...
                className={`flex min-h-12 w-full items-center gap-3 rounded-lg px-3 text-left text-sm font-semibold transition ${
                  active ? 'bg-[#f4dbc9] text-[#25160e]' : 'text-[#dec1b3] hover:bg-white/10 hover:text-white'
                }`}
              >
